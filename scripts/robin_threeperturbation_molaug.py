...
decoyset_fps = [
    ...
    for x in zinc_invitro_smiles_noredundant
]
...
df_sim_top = pd.read_csv(
    "rnadrug_dataset/df_sim_top15.csv", index_col=0, converters={"0": literal_eval}
)
...
for i in range(len(df_nohit_neg)):
    ...
    try:
        ...
        for ii in range(len(top15)):
            compound_iso_smiles.append(top15[ii])
            target_sequence.append(
                df_nohit_neg["target_sequence"][i]
                .strip()
                .replace("(m6A)", "")
                .replace(" and ", "")
            )
