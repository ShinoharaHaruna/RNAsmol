...
for i in range(len(train)):
    ...
    target_sequence.append(
        train["target_sequence"][i].strip().replace("(m6A)", "").replace(" and ", "")
    )
...
for i in range(len(val)):
    ...
    target_sequence.append(
        val["target_sequence"][i].strip().replace("(m6A)", "").replace(" and ", "")
    )
...
molaug_edge_set = (
    df_train_aug[["compound_iso_smiles", "target_sequence"]]
    .apply(tuple, axis=1)
    .tolist()
)

pdbrnaprotein_complete_edges = calculate_complete_interaction_remove_known_edges(
    pdbrnaprotein_node_set1, pdbrnaprotein_node_set2, pdbrnaprotein_edge_set
)
molaug_complete_edges = calculate_complete_interaction_remove_known_edges(
    molaug_node_set1, molaug_node_set2, molaug_edge_set
)

molaug_node_set1 = df_val_aug["compound_iso_smiles"].tolist()
molaug_node_set2 = df_val_aug["target_sequence"].tolist()
molaug_edge_set = (
    df_val_aug[["compound_iso_smiles", "target_sequence"]].apply(tuple, axis=1).tolist()
)

pdbrnaprotein_complete_edges_val = calculate_complete_interaction_remove_known_edges(
    pdbrnaprotein_node_set1, pdbrnaprotein_node_set2, pdbrnaprotein_edge_set
)
molaug_complete_edges_val = calculate_complete_interaction_remove_known_edges(
    molaug_node_set1, molaug_node_set2, molaug_edge_set
)
...
with open(outdir2 + "/raw/data_train.csv", "w+") as fcsv:
    ...
    for i in range(len(df_train_aug)):
        ...
        fcsv.write(
            df_train_aug["compound_iso_smiles"][i]
            + ","
            + df_train_aug["target_sequence"][i]
            + ","
            + "1"
            + "\n"
        )
        fcsv.write(
            df_train_aug["compound_iso_smiles"][i] + "," + neg + "," + "0" + "\n"
        )
        index += 1
...
with open(outdir2 + "/raw/data_val.csv", "w+") as fcsv:
    ...
    for i in range(len(df_val_aug)):
        ...
        fcsv.write(
            df_val_aug["compound_iso_smiles"][i]
            + ","
            + df_val_aug["target_sequence"][i]
            + ","
            + "1"
            + "\n"
        )
...
with open(outdir2 + "/raw/data_test.csv", "w+") as fcsv:
    ...
    for i in range(len(test)):
        ...
        fcsv.write(
            test["compound_iso_smiles"][i]
            + ","
            + test["target_sequence"][i]
            + ","
            + "1"
            + "\n"
        )
