...
chem2smiles = {
    chem_smiles["data"]["chem_comps"][i]["chem_comp"]["id"]: chem_smiles["data"][
        "chem_comps"
    ][i]["rcsb_chem_comp_descriptor"]["SMILES_stereo"]
    for i in range(len(chem_smiles["data"]["chem_comps"]))
}
...
def distance_between_ntbb_ligand(pdb_id, rnaseq, targetsmiles, ligname=None):
    ...
    # load the structure
    structure = PDB.MMCIFParser().get_structure(
        pdb_id, f"./ciffiles/{pdb_id.lower()}.cif"
    )
    # get the first model
    model = structure[0]
    bb_chains = {}
        ...
        for res in chain:
            if res.resname in backbone:
                ...
                if ligname:
                    ...
                    if Chem.MolToSmiles(Chem.MolFromSmiles(smiles)) == Chem.MolToSmiles(
