...
def get_binding_sites(rna, lig, suitable_lig_list, suitable_res_list):
    ...
    site_rna.filter(
        "index "
        + " ".join([str(index) for index in list(set(site_index_dict[selected_res]))])
    )
    return site_rna
...
if __name__ == "__main__":
    ...
    dataset_dict = pickle_load(
        os.path.join(BASE_PATH, "data/dataset_preparation/rna/rna_result_dicts.pkl")
    )
    dataset_df = pd.read_csv(
        os.path.join(BASE_PATH, "data/dataset_preparation/rna/rna_dataset_filter1.csv"),
        delimiter=",",
        index_col=None,
    )
    ...
    for mol_id in mol_ids:
        ...
        try:
            ...
            site_rna = get_binding_sites(
                rna, lig, ligand_dict[mol_id], res_dict[mol_id]
            )
