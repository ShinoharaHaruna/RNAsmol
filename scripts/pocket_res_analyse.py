            ...
            if res.id[1] == ligand.position:
                ...
                if is_valid_ligand(ligand_res):
                    valid_ligands.append(
                        (
                            ligand_res,
                            f"#{'0' if num_models == 1 else '0.1'}",
                            pocket_res_count(model, ligand_res),
                        )
                    )
                else:
                    invalid_ligands += 1
...
if __name__ == "__main__":
    ...
    process_all(
        "pdb_rna_ligand_20220923_onlyrna/cif_test",
        "pdb_rna_ligand_20220923_onlyrna/cif_test/test.p",
    )
