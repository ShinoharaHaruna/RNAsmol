...
def main():
    ...
    model = MCNN_GCN(
        3, 25 + 1, embedding_size=96, filter_num=32, out_dim=2, ban_heads=2
    ).to(device)
    ...
    model.load_state_dict(
        torch.load(
            "save/pdbrnaprotein_netshuffle_posneg11_rnafold_ppr_ban/model/epoch-0, loss-0.6926, val_loss-0.6962, test_loss-0.6956, test_acc-0.4615, test_sen-1.0000, test_spe-0.0000,test_pre-0.4615, test_rec-1.0000, test_f1-0.6316, test_rocauc-0.5442, test_prauc-0.4942, test_mcc-0.0000.pt",
