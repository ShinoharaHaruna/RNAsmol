...
riboaug_complete_edges = calculate_complete_interaction_remove_known_edges(
    riboaug_node_set1, riboaug_node_set2, riboaug_edge_set
)
...
temp = (
    df_train_aug.pivot(
        index="compound_iso_smiles", columns="target_sequence", values="affinity"
    )
    .reindex(index=vals, columns=vals)
    .fillna(0)
)

ss = (
    temp.rename_axis("compound_iso_smiles")
    .reset_index()
