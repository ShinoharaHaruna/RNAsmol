...
### only retain RNA
smm_na_sequence = smm_na_sequence[
    smm_na_sequence["Biomolecule Type"] == "RNA"
].reset_index(drop=True)

smm_target_hits = pd.read_csv("ROBIN/SMM_full_results/SMM_Target_Hits.csv")

### drop DNA targets
smm_target_hits.drop(
    columns=[
        "KRAS_hit",
        "MTOR_hit",
        "MYB_hit",
...
hit_mol_ind_list = smm_target_hits_new.loc[
    ~(smm_target_hits_new == 0.0).all(axis=1)
].index.tolist()
smm_target_hit_df = smm_target_hits.loc[
    smm_target_hits.index[hit_mol_ind_list]
].reset_index(drop=True)
...
for i in range(len(smm_target_hit_df)):
    for col in cols[2:]:
        ...
        target_sequence.append(
            smm_na_sequence["Sequence (5' to 3')"][ind]
            .replace("(m6A)", "")
            .replace(" and ", "")
        )
...
for i in range(len(df_pos)):
    ...
    posneg11_lst.append(
        df_pos["compound_iso_smiles"][i]
        + ","
        + df_pos["target_sequence"][i]
        + ","
        + "1"
    )
...
df_neg1 = pd.DataFrame(
    {
        ...
        "target_sequence": df_pos["target_sequence"].tolist(),
        "affinity": negaff1,
    }
)
df_neg2 = pd.DataFrame(
    {
        ...
        "target_sequence": df_pos["target_sequence"].tolist() * 2,
        "affinity": negaff2,
    }
)
df_neg3 = pd.DataFrame(
    {
        ...
        "target_sequence": df_pos["target_sequence"].tolist() * 5,
        "affinity": negaff3,
    }
)
