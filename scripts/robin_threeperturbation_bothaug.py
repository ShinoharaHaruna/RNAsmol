...
smm_na_sequence = pd.read_csv(
    "rnadrug_dataset/ROBIN/SMM_full_results/SMM_Sequence_Table_hit_rates.csv"
)
### only retain RNA
...
smm_target_hits = pd.read_csv(
    "rnadrug_dataset/ROBIN/SMM_full_results/SMM_Target_Hits.csv"
)
### drop DNA targets
        ...
        with open(afa_files[ind]) as f:
            ...
            for line in lines:
                if ">" not in line:
                    compound_iso_smiles.append(train["compound_iso_smiles"][i])
                    target_sequence.append(
                        line.strip().replace("(m6A)", "").replace(" and ", "")
                    )
                    affinity.append(train["affinity"][i])
    except:
        pass

df_train_aug_temp2 = pd.DataFrame(
    {
        "compound_iso_smiles": compound_iso_smiles,
        "target_sequence": target_sequence,
        "affinity": affinity,
...
df_train_aug = (
    pd.concat([df_train_aug_temp1, df_train_aug_temp2])
...
df_val_aug = (
    pd.concat([df_val_aug_temp1, df_val_aug_temp2])
    .drop("target_name", axis=1)
    .reset_index(drop=True)
)
...
df_proteinbinder_neg = pd.concat(
    [df_proteinbinder_neg, df_proteinbinder_neg]
).reset_index(drop=True)
