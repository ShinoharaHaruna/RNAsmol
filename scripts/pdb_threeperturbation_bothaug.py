...
pdbrnaprotein_nonredundent_df_pos = pd.read_csv(
    "datasets/pdb_rnaprotein_ligandsmile_rnaseq_affinity_pos_nonredundent",
    sep="\t",
    header=None,
)
...
for i in range(len(train)):
    ...
    try:
        ...
        top5 = mol_df.sort_values(by="similarity", ascending=False, inplace=False).head(
            5
        )
...
df_train_aug_temp1 = pd.DataFrame(
    {
...
df_train_aug = pd.concat([df_train_aug_temp1, df_train_aug_temp2]).reset_index(
    drop=True
)
...
bothaug_complete_edges = pd.DataFrame(
    bothaug_complete_edges,
    columns=["compound_iso_smiles", "target_sequence", "affinity"],
)
