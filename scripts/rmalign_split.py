...
for file in files:
    ...
    if reverse_file in file_set:
        ...
        try:
            with open(os.path.join(path, file), "r") as f1, open(
                os.path.join(path, reverse_file), "r"
            ) as f2:
...
df_ = pd.DataFrame(
    [(key[0], key[1], value) for key, value in score_dict.items()],
    columns=["pdbid1", "pdbid2", "Average RMscore"],
)
...
try:
    df_matrix = df_.pivot_table(
        index="pdbid1", columns="pdbid2", values="Average RMscore"
    )
