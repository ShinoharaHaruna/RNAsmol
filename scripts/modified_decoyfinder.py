...
calc_functs = {
    ...
    False: {
        ...
        "clogp": lambda mol: Decimal(Descriptors.MolLogP(mol)).quantize(
            Decimal("0.000")
        ),
...
def read_active_ligands(
    filename, echo_error=False, delimiter=" ", titleLine=True, use_original_props=False
):
    file = str(filename)
    active_ligands_comparable = []
    active_ligand_deposit = rdmolfiles.SmilesMolSupplier(
        data=file, delimiter=delimiter, titleLine=titleLine
    )
    ...
    print(
        "Found %d recognizable active ligands in input file."
        % (len(active_ligand_deposit))
    )
    print(
        "Generated %d comparable active ligands from input file."
        % (len(active_ligands_comparable))
    )
    return active_ligands_comparable


# Judge whether two molecules are too similar in chemical properties via Tanimoto coef.
def is_too_similar(ligand, db_mol, tanimoto_t=0.75):
...
def is_decoy(
    ligand, db_mol, HBA_t=2, HBD_t=1, ClogP_t=1.0, MW_t=25, RB_t=1, tanimoto_t=0.75
):
...
def parse_database(
    ...
    for db_file in db_file_list:
        ...
        generate_mol = rdmolfiles.SmilesMolSupplier(
            data=os.path.join(db_dir, db_file), delimiter=delimiter, titleLine=titleLine
        )
...
def make_decoy_dict(decoy_set):
    ...
    for key, value in decoy_set.items():
        fp_decoy_dict[key.name] = (fp2array(key), [fp2array(decoy) for decoy in value])
        smiles_decoy_dict[key.name] = (key.smiles, [decoy.smiles for decoy in value])
...
def make_run_log(ligands_dict, output_log_name, output_log_dir=""):
    df = pd.DataFrame(
        columns=["Name", "MW", "H_A", "H_D", "R_B", "cLogP", "Decoys_Found"]
    )
...
def find_decoys(
    ...
    unique=False,
    max_db_mols=1000000,
):  # max_db_mols: maximum number of db_mols to search; 0 if no restriction
    ...
    complete_ligand_sets = (
        0  # Number of active ligands with decoy set completed (at least #mind decoys)
    )
    ...
    for db_mol in db_generator:
        ...
        if db_mol_count % 10000 == 0:
            ...
            print(
                "%d decoy sets finished out of %d active ligands."
                % (complete_ligand_sets, nactive_ligands)
            )
            print(
                "%d decoys found in total; %d decoys required in total."
                % (sum(ligands_dict.values()), total_min)
            )
        ...
        if complete_ligand_sets >= nactive_ligands:
            # minreached = True
            print(
                "All decoy sets finished for %d active ligands. Search ends."
                % (nactive_ligands)
            )
            break
                        ...
                        if is_decoy(
                            ...
                            if decoy_set[ligand]:
                                for found_decoy in decoy_set[ligand]:
                                    if is_too_similar(
                                        found_decoy, db_mol, tanimoto_t=tanimoto_d
                                    ):
                            ...
                            if not too_similar_to_existing:
                                ...
                                ndecoys += 1
                                if (
                                    ligands_dict[ligand] >= mind
                                ):  # If decoy set complete for the active ligand
