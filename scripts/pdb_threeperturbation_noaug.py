...
pdbrnaprotein_nonredundent_df_pos = pd.read_csv(
    "rnadrug_dataset/pdb_rnaprotein_ligandsmile_rnaseq_affinity_pos_nonredundent",
    sep="\t",
    header=None,
)
pdbrnaprotein_nonredundent_df_pos.columns = [
    "compound_iso_smiles",
    "target_sequence",
    "affinity",
]
...
edge_set = (
    pdbrnaprotein_nonredundent_df_pos[["compound_iso_smiles", "target_sequence"]]
    .apply(tuple, axis=1)
    .tolist()
)
...
for node1 in node_set1:
    ...
    for node2 in node_set2:
        ...
        j = j + 1
    i = i + 1
complete_edges = pd.DataFrame(
    complete_edges, columns=["compound_iso_smiles", "target_sequence", "affinity"]
)
...
df_pdbrnaprotein_nohit_posneg11 = pd.concat(
    [pdbrnaprotein_nonredundent_df_pos, complete_edges.loc[nohit_ind_lst1]]
)
df_pdbrnaprotein_nohit_posneg12 = pd.concat(
    [pdbrnaprotein_nonredundent_df_pos, complete_edges.loc[nohit_ind_lst2]]
)
df_pdbrnaprotein_nohit_posneg15 = pd.concat(
    [pdbrnaprotein_nonredundent_df_pos, complete_edges.loc[nohit_ind_lst3]]
)
...
for i in range(len(pdbrnaprotein_nonredundent_df_pos)):
    neg = dinuclShuffle(
        pdbrnaprotein_nonredundent_df_pos["target_sequence"][i]
    ).replace("T", "U")
    neg2 = dinuclShuffle(
        pdbrnaprotein_nonredundent_df_pos["target_sequence"][i]
    ).replace("T", "U")
    ...
    posneg11_lst.append(
        pdbrnaprotein_nonredundent_df_pos["compound_iso_smiles"][i]
        + ","
        + pdbrnaprotein_nonredundent_df_pos["target_sequence"][i]
        + ","
        + "1"
...
pd.DataFrame([sub.split(",") for sub in test1], columns=uni_cols).to_csv(
    outdir1 + "/raw/data_test.csv", index=False
)
pd.DataFrame([sub.split(",") for sub in train1], columns=uni_cols).to_csv(
    outdir1 + "/raw/data_train.csv", index=False
)
pd.DataFrame([sub.split(",") for sub in val1], columns=uni_cols).to_csv(
    outdir1 + "/raw/data_val.csv", index=False
)
...
pd.DataFrame([sub.split(",") for sub in test2], columns=uni_cols).to_csv(
    outdir2 + "/raw/data_test.csv", index=False
)
pd.DataFrame([sub.split(",") for sub in train2], columns=uni_cols).to_csv(
    outdir2 + "/raw/data_train.csv", index=False
)
pd.DataFrame([sub.split(",") for sub in val2], columns=uni_cols).to_csv(
    outdir2 + "/raw/data_val.csv", index=False
)
...
pd.DataFrame([sub.split(",") for sub in test3], columns=uni_cols).to_csv(
    outdir3 + "/raw/data_test.csv", index=False
)
pd.DataFrame([sub.split(",") for sub in train3], columns=uni_cols).to_csv(
    outdir3 + "/raw/data_train.csv", index=False
)
pd.DataFrame([sub.split(",") for sub in val3], columns=uni_cols).to_csv(
    outdir3 + "/raw/data_val.csv", index=False
)


### rhom

df_proteinbinder = pd.read_csv(
    "rnadrug_dataset/robinrnabinder_bindingdbproteinbinder.csv"
)
df_proteinbinder_neg = df_proteinbinder[df_proteinbinder["target"] == 0.0].reset_index(
    drop=True
)
...
df_neg1 = pd.DataFrame(
    {
        "compound_iso_smiles": negsmile1,
        "target_sequence": pdbrnaprotein_nonredundent_df_pos[
            "target_sequence"
        ].tolist(),
        "affinity": negaff1,
    }
...
df_neg2 = pd.DataFrame(
    {
        "compound_iso_smiles": negsmile2,
        "target_sequence": pdbrnaprotein_nonredundent_df_pos["target_sequence"].tolist()
        * 2,
        "affinity": negaff2,
    }
)
df_neg3 = pd.DataFrame(
    {
        "compound_iso_smiles": negsmile3,
        "target_sequence": pdbrnaprotein_nonredundent_df_pos["target_sequence"].tolist()
        * 5,
        "affinity": negaff3,
    }
...
df_pdbrnaprotein_proteinbinder_posneg11 = pd.concat(
    [pdbrnaprotein_nonredundent_df_pos, df_neg1]
)
df_pdbrnaprotein_proteinbinder_posneg12 = pd.concat(
    [pdbrnaprotein_nonredundent_df_pos, df_neg2]
)
df_pdbrnaprotein_proteinbinder_posneg15 = pd.concat(
    [pdbrnaprotein_nonredundent_df_pos, df_neg3]
)
