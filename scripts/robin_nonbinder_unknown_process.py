...
def extract_data(target_hits, na_sequence):
    ...
    for i in range(len(target_hits)):
        for col in target_hits.columns[2:]:
            ...
            data["target_sequence"].append(
                na_sequence["Sequence (5' to 3')"][ind]
                .replace("U", "T")
                .replace("(m6A)", "")
                .replace(" and ", "")
            )
...
def main():
    ...
    df_pos, df_nohit_neg1, df_nohit_neg2, df_nohit_neg3 = process_and_save_data(
        smm_target_hits, smm_na_sequence, "/output/path"
    )
    ...
    for j, df in enumerate([df_nohit_neg1, df_nohit_neg2, df_nohit_neg3]):
        for k, neg_samples in enumerate(neg_samples_list):
            ...
            write_negative_samples(
                df,
                neg_samples,
                f"/output/path/robin_nohit_option{j}_{neg_sample_type}_{i}",
                "train",
            )
            write_negative_samples(
                df,
