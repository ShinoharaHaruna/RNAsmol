...
for idx, dir in enumerate(total_dirs):
    ...
    label_files_dict = {
        file.split("_epoch")[1].split("_test_label")[0]: file for file in label_files
    }
    pred_files_dict = {
        file.split("_epoch")[1].split("_test_pred")[0]: file for file in pred_files
    }
    ...
    for epoch in label_files_dict.keys():
        if epoch in pred_files_dict:
            ...
            ax.plot(
                ...
                prob_true,
                marker="o",
                label=f"{dbcat} model (epoch {epoch})",
                color="black",
                alpha=0.5,
            )
