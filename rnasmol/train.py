...
def main():
    ...
    parser.add_argument(
        "--save_model", action="store_true", help="whether save model or not"
    )
    ...
    train_loader = DataLoader(
        train_set, batch_size=params["batch_size"], shuffle=True, num_workers=8
    )
    val_loader = DataLoader(
        val_set, batch_size=params["batch_size"], shuffle=False, num_workers=8
    )
    test_loader = DataLoader(
        test_set, batch_size=params["batch_size"], shuffle=False, num_workers=8
    )

    device = torch.device("cuda:7" if torch.cuda.is_available() else "cpu")
    model = MCNN_GCN(
        3, 26, embedding_size=96, filter_num=32, out_dim=2, ban_heads=2
    ).to(device)
    ...
    for epoch in range(epochs):
        ...
        msg = (
            "epoch-%d, loss-%.4f, val_loss-%.4f, val_acc-%.4f, val_f1-%.4f, val_rocauc-%.4f, val_prauc-%.4f, val_mcc-%.4f, test_loss-%.4f, test_acc-%.4f, test_sen-%.4f, test_spe-%.4f,test_pre-%.4f, test_rec-%.4f, test_f1-%.4f, test_rocauc-%.4f, test_prauc-%.4f, test_mcc-%.4f"
            % (
                epoch,
                epoch_loss,
                val_loss,
