...
class StackCNN(nn.Module):
    def __init__(
        self, layer_num, in_channels, out_channels, kernel_size, stride=1, padding=0
    ):
        super().__init__()
...
class BANLayer(nn.Module):
    ...
    def forward(self, v, q, softmax=False):
        ...
        if self.h_out <= self.c:
            ...
            att_maps = (
                torch.einsum("xhyk,bvk,bqk->bhvq", (self.h_mat, v_, q_)) + self.h_bias
            )
            ...
            att_maps = self.h_net(
                d_.transpose(1, 2).transpose(2, 3)
            )  # b x v x q x h_out
...
class MCNN_GCN(nn.Module):
    def __init__(
        ...
        self.protein_encoder = TargetRepresentation(
            block_num, vocab_protein_size, embedding_size, dropout=0.5
        )
        ...
        self.ligand_encoder = GCNNet(
            num_features_xd=87, output_dim=filter_num * 3, dropout=0.5
        )
